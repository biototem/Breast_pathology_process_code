...
def xml_to_region(xml_file):
    ...
    for color in tree.findall('.//Annotation'):
        if True:#color.attrib['LineColor'] in ['65280','255','65535','16776960']:
            ...
            for region in color.findall('./Regions/Region'):
                ...
                region_list.append([vertex_list,color.attrib['LineColor']])
...
def region_handler(im, region_list,region_class, level_downsample):
    ...
    for r_class, [region,color] in enumerate(region_list):
        point_list = []
...
def region_free_hand_binary_image(tile, region_list,region_class, level_downsample,label_correction = True):
    """
...
def region_all_binary_image(tile,level_downsample,xml_file):
    # tree = ET.parse(xml_file)
    # region_list = []
    # region_class = []
    # boxes_coordinates = []
    # for annotation in tree.findall('.//Annotation'):
    #     if annotation.attrib['LineColor'] in ['65280','255','65535']:
    #         for region in annotation.findall('./Regions/Region'):
    #             region_type = region.attrib['Type']
    #             if region_type == '2':
    region_list,region_class = xml_to_region(xml_file)
    boxes_class = [ x == '2' or x == '1' for x in region_class]
    free_hand_class = [ x == '0' for x in region_class]
    if np.sum(free_hand_class) > 0:
        filter_matrix = region_free_hand_binary_image(tile, region_list,region_class, level_downsample,label_correction = True)
    else:
        filter_matrix = np.zeros((tile.size[1],tile.size[0]),dtype=np.uint8)
    
    if np.sum(boxes_class) > 0:
        for r_class, [region,color] in enumerate(region_list):
            x_list = []
            y_list = []
            if region_class[r_class] in ['2','1'] :
                for __, point in enumerate(region):
                    X, Y = int(float(point['X'])/level_downsample), int(float(point['Y'])/level_downsample)
                    y_list.append(Y)
                    x_list.append(X)
                h_begin,h_stop = max(min(y_list),0),min(max(y_list),tile.size[1]-1)
                w_begin,w_stop = max(min(x_list),0),min(max(x_list),tile.size[0]-1)
                filter_matrix[h_begin:h_stop,w_begin:w_stop] = 1
    return filter_matrix
...
def distinguish_left_and_right(xml_file):
    """
    parse XML label file and distinguish the left region and right region
    :param xml_file: xml file
    :return: region list,region_class
    """
    ...
    tree = ET.parse(xml_file)
    region_cord_list = {}
    x_cord_list,y_cord_list = [],[]
    for color in tree.findall('.//Annotation'):
        if color.attrib['LineColor'] in ['65280','255','65535']:
            # '65280'是绿色,'255'是红色,可以根据自己的实际情况更改这个判断条件(或者直接if True)
            for region in color.findall('./Regions/Region'):
                if region.attrib.get('Type')=='3':
                    for vertex in region.findall('.//Vertices/Vertex'):
                    # parse the 'X' and 'Y' for the vertex
                        x_cord_list.append(np.float32(vertex.attrib['X']))
                        y_cord_list.append(np.float32(vertex.attrib['Y']))
    if len(x_cord_list) > 0 and len(y_cord_list) > 0:
        region_cord_list['X']= np.average(x_cord_list)
        region_cord_list['Y']= np.average(y_cord_list)

    return region_cord_list
