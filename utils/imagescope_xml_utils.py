'''
软件 Aperio ImageScope 的 XML文件 读取和写入工具
        ...
        for ann in tree.findall('./Annotation'):
            ...
            for region in ann.findall('./Regions/Region'):
                ...
                if reg_type == TYPE_ARROW:
                    ...
                    arrow_points = np.asarray(arrow_head_tail_points, np.int32)
                    ...
                    box_points = np.asarray(box_points, np.int32)
                    ...
                    for vertex in region.findall('./Vertices/Vertex'):
                        ...
                        y = int(float(vertex.attrib['Y']))
                        ellipse.append((y, x))
                    ellipse = np.asarray(ellipse, np.int32)
